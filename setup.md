...
| **Create Transport Requests Before Transport Orders** | Controls whether **Create Transport Order** on source documents can create or prepare Transport Requests first | Off for planner-controlled request release; on for one-click order creation | Turn it on when users should not manage Transport Requests separately. Missing requests are created in **Released** status, and existing **Open** requests are released and included. When off, only existing **Released** requests are used and **Open** requests must be released manually |
